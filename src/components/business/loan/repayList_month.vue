<template>
	<div>
		...
		<section class='loan_tabs' :value="initData">
			<Tabs type="card">
				<TabPane label="全部">
					<p>应还资方金额：{{initData.oughtReplayAmount || 0}}</p>
					<p class="all_bank">
						<span>众安：        <em>{{totalLoan || 0}}</em></span>
					</p>
					<p class="all_details">
						<span>本月应收账款：   <em>{{initData.planReplayAmount || 0}}</em> </span>
					...
					<p class="all_details">
						<span>本月提前还款案件：    <em>{{initData.advanceReplayNum || 0}}</em></span>
						<span>提前还款金额：        <em>{{initData.advanceReplayAmount || 0}}</em></span>
						<span>本月提前结清案件数：  <em>{{initData.advanceClearCaseNum || 0}}</em></span>
						<span>本月提前结清金额：    <em>{{initData.advanceReplayAmount || 0}}</em></span>
					</p>
				</TabPane>
				<TabPane label="众安">
					<p>应还资方金额：<em>{{initData.loanAmount || 0}}</em></p>
					<p class="all_details">
						<span>本月应收账款：   <em>{{initData.planReplayAmount || 0}}</em> </span>
						<span>实际到账金额：    <em>{{initData.actualReplayAmount || 0}}</em></span>
						<span>未到账金额：       <em>{{initData.unReplayAmount || 0}}</em></span>
		...
		<section class="loan_table">
			...
			<el-table :data="datatable" border>
				...
				<el-table-column prop="termNo" label="期号" min-width="130">
				</el-table-column>
				...
				<el-table-column prop="actualServiceAmount" label="实际还款平台服务费" min-width="100">
				</el-table-column>
...
//					let months = d.getFullYear() + '-' + (d.getMonth() + 1);
//					this.value5 = months;　　　　　　　　　　
//				},
//				　　　　　　　　　　deep: true　　　　　　　　
//			}
		...
		methods: {
			...
			handleDownload() {
				...
				require.ensure([], () => {
					...
					const tHeader = ['还款编号', '申请编号', '放款编号', '期号', '用户号', '用户姓名', '证件号', '计划还款时间', '计划还款本金', '计划还款利息', '计划还款平台', '实际还款时间', '实际还款本金', '实际还款利息', '实际还款平台', '还款状态', '逾期天数', '逾期费', '扣款流水号'];
					...
					export_json_to_excel(tHeader, data, '按月导出还款列表');
			...
			getData() {
				if(this.value5 == "" ) {
					...
					var m = (day1.getMonth()+1).toString();
							   m = m.length > 1 ? m : "0"+m;
					var s1 = day1.getFullYear() + "-" + m ;
                     this.value5 = s1;
				}
				let start = this.value5 + "-" + 1;
				let end = this.value5 + "-" + 31;
...
//					console.log('@@@@@@@@@@@loanReplayAmount_月=========', this.initData)
